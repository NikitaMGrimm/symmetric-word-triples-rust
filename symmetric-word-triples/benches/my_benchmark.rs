...
use symmetric_word_triples::{auto_dir_sym_word_sol, auto_single_sym_word_sol};
...
pub fn benchmark_dict_big(c: &mut Criterion) {
    ...
    c.bench_function("dir symmetry bench", move |b| {
        b.iter(|| {
            auto_dir_sym_word_sol(&input_dir, &output_dir, grid_range, chunk_size_range, true).unwrap()
        })
    });
}
criterion_group!{
